...
export default function ParentZoneHubPage() {
  return (
    ...
    >
      <section className="clay-container rounded-3xl border border-indigo-100 bg-white/95 p-5 shadow-sm sm:p-6">
        <div className="space-y-3">
          {PARENT_ZONE_ACTIVITIES.map((activity) => (
            <Link
              key={activity.id}
              to={activity.to}
              className="clay-card group flex items-center justify-between rounded-2xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-sky-50 px-4 py-3 transition hover:-translate-y-0.5 hover:from-indigo-100 hover:to-sky-100"
            >
